...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
            ...
            InlineKeyboardButton('💙 𝗔𝗻𝗶𝗺𝗲𝘀', url="https://t.me/AnimesWorldTamil"),
            InlineKeyboardButton('🎬 𝐌𝐨𝐯𝐢𝐞 𝐆𝐫𝐨𝐮𝐩', url='https://t.me/Dp_botz')
        ],[
    ...
    if len(message.command) != 2:
        if PREMIUM_AND_REFERAL_MODE == True:
            buttons = [[
                ...
                InlineKeyboardButton('💰 𝐆𝐞𝐭 𝐅𝐫𝐞𝐞 /𝐏𝐚𝐢𝐝 𝐒𝐮𝐛𝐬𝐜𝐫𝐢𝐩𝐭𝐢𝐨𝐧 💰', callback_data='subscription')
            ],[
                InlineKeyboardButton('⚡ 𝐉𝐨𝐢𝐧 𝐔𝐩𝐝𝐚𝐭𝐞𝐬 𝐂𝐡𝐚𝐧𝐧𝐞𝐥 ⚡', url=CHNL_LNK)
            ]]
        else:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_sticker("CAACAgUAAxkBAAIBIWZPECLAwM9Y8M3j6vFDKvCRAAGldwACCgUAAjWEuVU73d11wZSdoh4E") 
            ...
            try:
                ...
                if STREAM_MODE == True:
                    button = [[
                        ...
                        InlineKeyboardButton('𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥', url="https://t.me/mersal_dhinesh")
                    ],[
        ...
        k = await client.send_message(chat_id=message.from_user.id,text=f"<b>Get All Files in a Single Click!!!\n\n📂 ʟɪɴᴋ ➠ : {g}\n\n<i>Note: This message is deleted in 5 mins to avoid copyrights. Save the link to Somewhere else</i></b>", reply_markup=InlineKeyboardMarkup(
                [
                    [
                        ...
                        InlineKeyboardButton('🤔 𝐇𝐨𝐰 𝐓𝐨 𝐃𝐨𝐰𝐧𝐥𝐨𝐚𝐝 🤔', url=await get_tutorial(chat_id))
                    ]
                ]
            )
        )
        ...
        for file in files:
            ...
            if not await db.has_premium_access(message.from_user.id):
                if not await check_verification(client, message.from_user.id) and VERIFY == True:
                    btn = [[
                        InlineKeyboardButton("𝐕𝐞𝐫𝐢𝐟𝐲", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
                    ],[
                        InlineKeyboardButton("𝐇𝐨𝐰 𝐓𝐨 𝐎𝐩𝐞𝐧 𝐋𝐢𝐧𝐤 & 𝐕𝐞𝐫𝐢𝐟𝐲", url=VERIFY_TUTORIAL)
                    ]]
                    await message.reply_text(
            ...
            if STREAM_MODE == True:
                button = [[
                    ...
                    InlineKeyboardButton('💙𝐔𝐩𝐝𝐚𝐭𝐞𝐬💙', url=CHNL_LNK)
                ],[
                    InlineKeyboardButton("𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥", url="t.me/mersal_dhinesh")
                ],[
                    InlineKeyboardButton('🚀 Fast Download / Watch Online🖥️', callback_data=f'generate_stream_link:{file_id}') #Don't change anything without contacting me @mersal_dhinesh
                ]]
            else:
                button = [[
                    InlineKeyboardButton('💥𝐒𝐮𝐩𝐩𝐨𝐫𝐭💥', url=f'https://t.me/{SUPPORT_CHAT}'),
    ...
    if not files_:
        ...
        try:
            ...
            title = '@Movies_Days  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@Movies_Days  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@Movies_Days  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            btn = [[
